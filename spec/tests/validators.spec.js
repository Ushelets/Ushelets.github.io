...
describe('Проверка модуля validators на сервере:', () => {
     ...
     /* it('Проверьте длину пароля!', () => {
            ...
            expect(valid.Auth(modelServer[4].email, modelServer[4].password).errors).toEqual({ passwordLength: 'Длина пароля должна быть не менее 6 символов!' })
      });*/
     /* it('Пароль не верен!', () => {
            expect(valid.Auth(modelServer[4].email, modelServer[4].password).isValid).toBe(false);
            expect(valid.Auth(modelServer[4].email, modelServer[4].password).errors).toEqual({ different: 'Пароли не совпадают!' })
      })*/
});